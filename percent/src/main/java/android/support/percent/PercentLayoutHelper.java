...
public class PercentLayoutHelper {
    ...
    private final int mWidthPixels, mHeightPixels;
    ...
    public PercentLayoutHelper(@NonNull ViewGroup host) {
        ...
        mHost = host;
        DisplayMetrics metrics = host.getContext().getResources().getDisplayMetrics();
        mWidthPixels = metrics.widthPixels;
        mHeightPixels = metrics.heightPixels;
        if (DEBUG) {
            Log.d(TAG, "mWidthPixels: " + mWidthPixels + " mHeightPixels: " + mHeightPixels);
        }
    }
    ...
    public void adjustChildren(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0, N = mHost.getChildCount(); i < N; i++) {
            ...
            if (params instanceof PercentLayoutParams) {
                ...
                if (info != null) {
                    if (info.percentBasedOn == 1) {
                        if (DEBUG) {
                            Log.d(TAG, "percent based on window, adjust widthHint and heightHint");
                        }
                        widthHint = mWidthPixels;
                        heightHint = mHeightPixels;
                    }
    ...
    /**
     * parse the dimensionRatio
     *
     * @param dimensionRatio the dimensionRatio string, maybe 1:2 or 0.5
     * @return the dimensionRatio float value range at 0-1, or -1 if parse error
     */
    private static float parseDimensionRatio(String dimensionRatio) {
        float dimensionRatioValue = -1f;
        ...
        if ((colonIndex >= 0) && (colonIndex < len - 1)) {
            String nominator = dimensionRatio.substring(0, colonIndex);
            ...
            if (len > 0) {
                try {
                    dimensionRatioValue = Float.parseFloat(dimensionRatio);
                } catch (NumberFormatException ignored) {
                }
            }
        }
        if (dimensionRatioValue > 1f) {
            dimensionRatioValue = 1f;
        }
    ...
    public static PercentLayoutInfo getPercentLayoutInfo(Context context,
        ...
        String dimensionRatio = array.getString(R.styleable.PercentLayout_Layout_layout_widthDimensionRatio);
        float value = parseDimensionRatio(dimensionRatio);
        if (value != -1f) {
            if (VERBOSE) {
                Log.v(TAG, "dimension ratio width: " + value);
            }
            info = info != null ? info : new PercentLayoutInfo();
            info.widthPercent = value;
        } else {
            value = array.getFraction(R.styleable.PercentLayout_Layout_layout_widthPercent, 1, 1,
                    -1f);
            if (value != -1f) {
                if (VERBOSE) {
        ...
        dimensionRatio = array.getString(R.styleable.PercentLayout_Layout_layout_heightDimensionRatio);
        value = parseDimensionRatio(dimensionRatio);
        if (value != -1f) {
            if (VERBOSE) {
                Log.v(TAG, "dimension ratio height: " + value);
            }
            info = info != null ? info : new PercentLayoutInfo();
        ...
        dimensionRatio = array.getString(R.styleable.PercentLayout_Layout_layout_marginDimensionRatio);
        value = parseDimensionRatio(dimensionRatio);
        if (value != -1f) {
            if (VERBOSE) {
                Log.v(TAG, "dimension ratio margin: " + value);
            }
            info = info != null ? info : new PercentLayoutInfo();
        ...
        dimensionRatio = array.getString(R.styleable.PercentLayout_Layout_layout_marginLeftDimensionRatio);
        value = parseDimensionRatio(dimensionRatio);
        if (value != -1f) {
            if (VERBOSE) {
                Log.v(TAG, "dimension ratio left margin: " + value);
            }
            info = info != null ? info : new PercentLayoutInfo();
            info.leftMarginPercent = value;
        } else {
        ...
        dimensionRatio = array.getString(R.styleable.PercentLayout_Layout_layout_marginTopDimensionRatio);
        value = parseDimensionRatio(dimensionRatio);
        if (value != -1f) {
            if (VERBOSE) {
                Log.v(TAG, "dimension ratio top margin: " + value);
            }
            info = info != null ? info : new PercentLayoutInfo();
            info.topMarginPercent = value;
        } else {
        ...
        dimensionRatio = array.getString(R.styleable.PercentLayout_Layout_layout_marginRightDimensionRatio);
        value = parseDimensionRatio(dimensionRatio);
        if (value != -1f) {
            if (VERBOSE) {
                Log.v(TAG, "dimension ratio right margin: " + value);
            }
            info = info != null ? info : new PercentLayoutInfo();
            info.rightMarginPercent = value;
        } else {
        ...
        dimensionRatio = array.getString(R.styleable.PercentLayout_Layout_layout_marginBottomDimensionRatio);
        value = parseDimensionRatio(dimensionRatio);
        if (value != -1f) {
            if (VERBOSE) {
                Log.v(TAG, "dimension ratio bottom margin: " + value);
            }
            info = info != null ? info : new PercentLayoutInfo();
            info.bottomMarginPercent = value;
        } else {
        ...
        dimensionRatio = array.getString(R.styleable.PercentLayout_Layout_layout_marginStartDimensionRatio);
        value = parseDimensionRatio(dimensionRatio);
        if (value != -1f) {
            if (VERBOSE) {
                Log.v(TAG, "dimension ratio start margin: " + value);
            }
            info = info != null ? info : new PercentLayoutInfo();
        ...
        dimensionRatio = array.getString(R.styleable.PercentLayout_Layout_layout_marginEndDimensionRatio);
        value = parseDimensionRatio(dimensionRatio);
        if (value != -1f) {
            if (VERBOSE) {
                Log.v(TAG, "dimension ratio end margin: " + value);
            }
            info = info != null ? info : new PercentLayoutInfo();
        ...
        int intValue = array.getInt(R.styleable.PercentLayout_Layout_layout_percentBasedOn, 0);
        info = info != null ? info : new PercentLayoutInfo();
        info.percentBasedOn = intValue;
    ...
    public static class PercentLayoutInfo {
        ...
        /** Indicates whether all the percentage-based values is based on who. 0 - parent, 1- window */
        public int percentBasedOn;
